...
print(f"Begin certification base model: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
plot_certified_acc(model_base, X, y, num_samples_estimation=10000,
                       num_samples_selection=100, plot_file=run_dir / "model_base")
print(f"End certification base model: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)

print(f"Begin certification adversarial model: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
plot_certified_acc(model_adv, X, y, num_samples_estimation=10000,
